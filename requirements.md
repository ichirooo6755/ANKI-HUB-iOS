...
ChapterSelection の章 = `category`。`先生・` / `AI・` プレフィックスで選択可能。件数: **先生 約259問 / AI 約30問**（計297・2026-07-28）。

| 区分 | 対象（id） | 判断 |
|------|------------|------|
| **先生** | `mf-r` respon / `mf-p` 練習 / `mf-t` 授業内クイズ / `mf-h01–16` 後半練習 / `mf-uc` 理解度チェック / `mf-fc` 第22–26重点 / `mf-h17–24`（第26回まとめへ再分類） / `mf-m` 中間OCR / `mf-f` 期末OCR / `mf-pdf` 期末PDF | Manaba教材の小テスト・練習・respon解答・高見澤等の授業内クイズ・理解度チェック・第22–26回重点・実試験写真OCR・**手元PDF**など **教材・試験由来** |
| **AI** | `mf-q`・`mf-e` 概念確認 / `mf-s` 基礎補強 | 講義スライド等からエージェントが作った **補強・概念確認カード**（オリジナルMCQ） |
...
### マネファイ 理解度チェック・第22–26回重点（2026-07-28）

| 項目 | 内容 |
|------|------|
| 件数 | **246 → 297問**（+51、既存8問をAI→先生再分類） |
| 第16–20 | 章 `先生・理解度チェック（第16–20回）`。スライド講義根拠＋第20回スライドクイズ。既存 `先生・後半・クイズ` / `練習問題` と stem+choices dedupe（重複0） |
| 第22–26 | 章 `先生・第22–26回重点`（第22/24/25まとめ）・`先生・第26回まとめ`（金融政策・IS-LM・QQE/YCC/QT）。`mf-h17–24` を AI 章から先生章へ移行 |
| Manaba小テスト | storageState で `course_1963831_query` を試行 → **ERR_TOO_MANY_REDIRECTS**（セッション失効）。解答公開ページ未取得 |
| スクリプト | `python3 scripts/manaba_extract/merge_mf_check_16_26.py` |
| ビルド | 実機 `platform=iOS,name=iPhoneA13` **BUILD SUCCEEDED**（install は devicectl 接続タイムアウト） |
